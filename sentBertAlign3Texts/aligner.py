...
class Bertalign:
    def __init__(self,
                 ...
                 len_penalty=True,
                 is_split=False,
                 fix_src_lines=False,  # do not split src lines further; changed to accommodate self.align_sents_1toN()
                 src_lang=None,
                 tgt_lang=None,
               ):
        ...
        if fix_src_lines:
            # simply treat lines as-is without further splitting them
            src_sents = [s.strip() for s in src.splitlines() if s.strip() != '']
    ...
    def align_sents_1toN(self):
        ...
        print("Performing second-step alignment ...")
        second_alignment_types = get_alignment_types(self.max_align)
        second_alignment_types = [[i, j] for [i, j] in second_alignment_types if i < 2]  # remove [m,n] where m > 1
        second_alignment_types = np.array(second_alignment_types)
